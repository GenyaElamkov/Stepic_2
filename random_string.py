...
def area_figure():
    ...
    """
    n = 10 ** 6     # Кол-во испытаний.
    k = 0           # Исходные точки внутри фигуры.
    s0 = 16         # Площадь квадрата или прямоугольника.
    for _ in range(n):
        x = uniform(-2, 2)
        y = uniform(-2, 2)
...
def pi_area():
    ...
    n = 10 ** 6                         # Кол-во испытаний.
    k = 0                               # Исходные точки внутри фигуры.
    s0 = 4                              # Площадь квадрата или прямоугольника.
    for _ in range(n):
        x = uniform(-1, 1)              # случайное число с плавающей точкой от 0 до 1
        y = uniform(-1, 1)              # случайное число с плавающей точкой от 0 до 1
